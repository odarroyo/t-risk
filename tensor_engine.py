...
def generate_synthetic_portfolio(n_assets: int, n_events: int, n_typologies: int = 5,
    ...
    lambdas : np.ndarray, optional
        Pre-specified scenario occurrence rates $\in \mathbb{R}^Q$. If None, will be generated
    ...
    v_exposure : np.ndarray, shape (N,)
        Exposure vector containing replacement costs for each asset $\in \mathbb{R}^N$.
        Values range from 100,000 to 1,000,000
    u_typology : np.ndarray, shape (N,)
        Typology index vector mapping each asset to a vulnerability curve $\in \mathbb{Z}^N$.
        Values in {0, 1, ..., K-1}
    C_matrix : np.ndarray, shape (K, M)
        Vulnerability matrix containing K curves with M points each $\in \mathbb{R}^{K \times M}$.
        Each curve represents Mean Damage Ratio as function of intensity
    x_grid : np.ndarray, shape (M,)
        Common intensity grid vector for all curves $\in \mathbb{R}^M$.
        Values range from 0.0g to 1.5g
    H_intensities : np.ndarray, shape (N, Q)
        Hazard intensity matrix $\in \mathbb{R}^{N \times Q}$.
        H[i,q] = ground motion intensity at asset i during event q
    lambdas_out : np.ndarray, shape (Q,)
        Scenario occurrence rate vector $\in \mathbb{R}^Q$.
        $\lambda_q$ = annual occurrence rate of event q (events/year)
    Sigma_matrix : np.ndarray, shape (K, M) or None
        Vulnerability std dev matrix $\in \mathbb{R}^{K \times M}$, or None if sigma_fraction not set
...
def deterministic_loss(v: tf.Tensor, u: tf.Tensor, C: tf.Tensor, 
    ...
    - Eq. 1: $\alpha = (h - x_j) / (x_{j+1} - x_j)$
    - Eq. 2: $\text{MDR}_i = (1-\alpha) \cdot C[u_i,j] + \alpha \cdot C[u_i,j+1]$
    - Eq. 3: $J = \sum v_i \cdot \text{MDR}_i$
    ...
    v : tf.Tensor, shape (N,), dtype float32
        Exposure vector containing replacement costs for N assets $\in \mathbb{R}^N$
    u : tf.Tensor, shape (N,), dtype int32
        Typology index vector mapping assets to vulnerability curves $\in \mathbb{Z}^N$.
        Values must be in range [0, K-1]
    C : tf.Tensor, shape (K, M), dtype float32
        Vulnerability matrix with K curves and M intensity points $\in \mathbb{R}^{K \times M}$.
        C[k,m] represents Mean Damage Ratio for typology k at intensity x[m]
    x_grid : tf.Tensor, shape (M,), dtype float32
        Intensity grid vector defining curve discretization $\in \mathbb{R}^M$.
        Must be monotonically increasing
    h : tf.Tensor, shape (N,), dtype float32
        Intensity vector for single scenario $\in \mathbb{R}^N$.
    ...
    --------------------
    For each asset i:
    1. Find grid index j such that $x[j] \le h[i] < x[j+1]$
    2. Compute interpolation weight $\alpha = (h[i] - x[j]) / (x[j+1] - x[j])$
    3. Interpolate damage: $\text{MDR}[i] = (1-\alpha) \cdot C[u[i],j] + \alpha \cdot C[u[i],j+1]$
    4. Compute asset loss: $L[i] = v[i] \cdot \text{MDR}[i]$
    5. Aggregate: $J = \sum L[i]$
...
def probabilistic_loss_matrix(v: tf.Tensor, u: tf.Tensor, C: tf.Tensor,
    ...
    $J_{i,q} = v_i \times [(1-\alpha_{i,q}) \cdot C[u_i,j_{i,q}] + \alpha_{i,q} \cdot C[u_i,j_{i,q}+1]]$
    
    This function extends deterministic loss to handle Q stochastic event
    realizations simultaneously, computing the complete $N \times Q$ loss matrix in a
    single vectorized operation.
    
    Parameters
    ----------
    v : tf.Tensor, shape (N,), dtype float32
        Exposure vector for N assets $\in \mathbb{R}^N$
    u : tf.Tensor, shape (N,), dtype int32
        Typology index vector $\in \mathbb{Z}^N$
    C : tf.Tensor, shape (K, M), dtype float32
        Vulnerability matrix $\in \mathbb{R}^{K \times M}$
    x_grid : tf.Tensor, shape (M,), dtype float32
        Intensity grid vector $\in \mathbb{R}^M$
    H : tf.Tensor, shape (N, Q), dtype float32
        Hazard intensity matrix $\in \mathbb{R}^{N \times Q}$
    ...
    J_matrix : tf.Tensor, shape (N, Q), dtype float32
        Loss matrix where J[i,q] is the loss of asset i in event q $\in \mathbb{R}^{N \times Q}$
    ...
    Mathematical Details
    --------------------
    The function processes all $N \times Q$ combinations simultaneously:
    1. Flatten H to vector of length N*Q
    2. Find grid indices for all intensities
    3. Compute interpolation weights $\alpha$ for all combinations
...
def compute_risk_metrics(J_matrix: tf.Tensor, lambdas: Optional[tf.Tensor] = None) -> Dict[str, tf.Tensor]:
    ...
    - $\text{AAL}_i = \sum_q \lambda_q \cdot J_{i,q}$  (rate-weighted annual loss)
    - $\mu_i = \text{AAL}_i / \Lambda = \sum_q w_q \cdot J_{i,q}$  (mean per event)
    - $\sigma^2_i = \sum_q w_q \cdot (J_{i,q} - \mu_i)^2$  (rate-weighted variance)
    
    Parameters
    ----------
    J_matrix : tf.Tensor, shape (N, Q), dtype float32
        Loss matrix where J[i,q] is loss of asset i in event q $\in \mathbb{R}^{N \times Q}$
    lambdas : tf.Tensor, shape (Q,), dtype float32, optional
        Scenario occurrence rate vector $\in \mathbb{R}^Q$ where $\lambda_q \ge 0$
    ...
    metrics : dict
        ...
        aal_per_asset : tf.Tensor, shape (N,), dtype float32
            Rate-weighted Average Annual Loss per asset $\in \mathbb{R}^N$.
            $\text{AAL}_i = \sum_q \lambda_q \cdot J_{i,q}$
            
        aal_portfolio : tf.Tensor, scalar, dtype float32
            Total portfolio Average Annual Loss.
            $\text{AAL}_{\text{portfolio}} = \sum_i \text{AAL}_i$
            
        mean_per_event_per_asset : tf.Tensor, shape (N,), dtype float32
            Mean loss per event occurrence per asset $\in \mathbb{R}^N$.
            $\mu_i = \text{AAL}_i / \Lambda = \sum_q w_q \cdot J_{i,q}$
            
        variance_per_asset : tf.Tensor, shape (N,), dtype float32
            Rate-weighted variance of loss per asset $\in \mathbb{R}^N$.
            $\sigma^2_i = \sum_q w_q \cdot (J_{i,q} - \mu_i)^2$
            
        std_per_asset : tf.Tensor, shape (N,), dtype float32
            Standard deviation of loss per asset $\in \mathbb{R}^N$.
            $\sigma_i = \sqrt{\sigma^2_i}$
            
        loss_per_event : tf.Tensor, shape (Q,), dtype float32
            Total portfolio loss for each event $\in \mathbb{R}^Q$.
            $L_q = \sum_i J_{i,q}$
            
        total_rate : tf.Tensor, scalar, dtype float32
            Total occurrence rate $\Lambda = \sum_q \lambda_q$
    
    Notes
    -----
    - All computations are differentiable w.r.t. J_matrix and lambdas
    - AAL represents rate-weighted expected annual loss
    - Variance is weighted by normalized rates $w_q = \lambda_q / \Lambda$
    - When lambdas is uniform ($\lambda_q = 1/Q$), reduces to simple averaging
...
def compute_risk_metrics_with_uncertainty(
    ...
    - Aleatory (event) variance: $\text{Var}_{\text{events}}[E[\text{Loss} | \text{event}]]$
    - Vulnerability variance: $E_{\text{events}}[\text{Var}_{\text{vuln}}[\text{Loss} | \text{event}]]$

    Using the law of total variance:

    $\text{Var}_{\text{total}} = \text{Var}_{\text{aleatory}} + \text{Var}_{\text{vulnerability}}$

    $\text{Var}_{\text{vuln},i} = \sum_q w_q \cdot v_i^2 \cdot \sigma^2[u_i, m(i,q)]$

    Parameters
    ----------
    J_matrix : tf.Tensor, shape (N, Q), dtype float32
        Loss matrix $\in \mathbb{R}^{N \times Q}$
    lambdas : tf.Tensor, shape (Q,), dtype float32
        Scenario occurrence rates $\in \mathbb{R}^Q$
    v : tf.Tensor, shape (N,), dtype float32
        Exposure vector $\in \mathbb{R}^N$
...
class TensorialRiskEngine:
    ...
    v : tf.Variable, shape (N,), dtype float32
        Exposure vector (replacement costs) $\in \mathbb{R}^N$
    ...
    gradient_wrt_vulnerability()
        Compute $\partial(\text{AAL})/\partial C$ (Manuscript Section 4)
    gradient_wrt_exposure()
        Compute $\partial(\text{AAL})/\partial v$ (Manuscript Section 5)
    gradient_wrt_hazard()
        Compute $\partial(\text{AAL})/\partial H$ (Manuscript Section 5)
    full_gradient_analysis()
        Compute complete gradient $\nabla J = [\partial J/\partial H, \partial J/\partial C, \partial J/\partial v]$
    ...
    def __init__(self, v: np.ndarray, u: np.ndarray, C: np.ndarray,
        ...
        u : np.ndarray, shape (N,)
            Typology index vector $\in \mathbb{Z}^N$, values in {0, ..., K-1}
        ...
        Sigma : np.ndarray, shape (K, M), optional
            Vulnerability standard deviation matrix $\in \mathbb{R}^{K \times M}$.
            Sigma[k,m] = std dev of MDR at intensity x_grid[m] for typology k.
            Must satisfy $\Sigma^2 < C \times (1 - C)$ for Beta distribution validity.
    ...
    def compute_loss_and_metrics(self) -> Tuple[tf.Tensor, Dict]:
        ...
        $N \times Q$ loss matrix, then computes all derived risk metrics using
        scenario occurrence rates (Manuscript Section 3c).
        
        Returns
        -------
        J_matrix : tf.Tensor, shape (N, Q), dtype float32
            Complete loss matrix $\in \mathbb{R}^{N \times Q}$
            J[i,q] = loss of asset i in event q
        metrics : dict
            Dictionary of rate-weighted risk metrics containing:
            - aal_per_asset : Rate-weighted AAL per asset $\in \mathbb{R}^N$
            - aal_portfolio : Total portfolio AAL (scalar)
            - mean_per_event_per_asset : Mean per event $\in \mathbb{R}^N$
            - variance_per_asset : Rate-weighted variance $\in \mathbb{R}^N$
            - std_per_asset : Standard deviation $\in \mathbb{R}^N$
            - loss_per_event : Total loss per event $\in \mathbb{R}^Q$
            - total_rate : Total occurrence rate $\Lambda$
        ...
        - variance_vulnerability_per_asset : Vulnerability variance component $\in \mathbb{R}^N$
        - variance_vulnerability_portfolio : Portfolio vulnerability variance (scalar)
        - variance_total_per_asset : Total variance (aleatory + vulnerability) $\in \mathbb{R}^N$
        - std_total_per_asset : Total std dev $\in \mathbb{R}^N$
    ...
    def gradient_wrt_vulnerability(self) -> Tuple[tf.Tensor, Dict]:
        ...
        Calculates $\partial(\text{AAL})/\partial C$ using automatic differentiation, answering the question:
        "How sensitive is portfolio risk to changes in each vulnerability curve point?"
        
        Uses rate-weighted formulation: $\text{AAL} = \sum_q \lambda_q \cdot \sum_i J_{i,q}$
        ...
        grad_C : tf.Tensor, shape (K, M), dtype float32
            Gradient of portfolio AAL w.r.t. vulnerability matrix $\in \mathbb{R}^{K \times M}$.
            $\text{grad\_C}[k,m] = \partial(\text{AAL})/\partial C[k,m]$
    ...
    def gradient_wrt_exposure(self) -> Tuple[tf.Tensor, Dict]:
        ...
        Calculates $\partial(\text{AAL})/\partial v$ using automatic differentiation, answering the question:
        "How much does portfolio AAL increase per additional dollar of exposure at each asset?"
        
        Uses rate-weighted formulation: $\partial\text{AAL}/\partial v_i = \sum_q \lambda_q \cdot \text{MDR}[i,q]$
        ...
        grad_v : tf.Tensor, shape (N,), dtype float32
            Gradient of portfolio AAL w.r.t. exposure vector $\in \mathbb{R}^N$.
            $\text{grad\_v}[i] = \partial(\text{AAL})/\partial v[i]$
    ...
    def gradient_wrt_hazard(self) -> Tuple[tf.Tensor, Dict]:
        ...
        Calculates $\partial(\text{AAL})/\partial H$ using automatic differentiation, answering the question:
        "How sensitive is portfolio risk to changes in hazard intensity estimates?"
        
        Uses rate-weighted formulation: $\partial\text{AAL}/\partial H[i,q]$ includes $\lambda_q$ factor
        ...
        grad_H : tf.Tensor, shape (N, Q), dtype float32
            Gradient of portfolio AAL w.r.t. hazard matrix $\in \mathbb{R}^{N \times Q}$.
            $\text{grad\_H}[i,q] = \partial(\text{AAL})/\partial H[i,q]$
    ...
    def gradient_wrt_lambdas(self) -> Tuple[tf.Tensor, Dict]:
        ...
        Calculates $\partial(\text{AAL})/\partial\lambda$ using automatic differentiation, answering the question:
        ...
        grad_lambdas : tf.Tensor, shape (Q,), dtype float32
            Gradient of portfolio AAL w.r.t. scenario rates $\in \mathbb{R}^Q$
            $\text{grad\_lambdas}[q] = \partial(\text{AAL})/\partial\lambda_q = \sum_i J_{i,q}$
        ...
        Notes
        -----
        - Gradient is simply the loss per event: $\partial(\sum_q \lambda_q \cdot L_q)/\partial\lambda_q = L_q$
    ...
    def gradient_wrt_sigma(self) -> Tuple[tf.Tensor, Dict]:
        ...
        Calculates $\partial(\text{Var}_{\text{total}})/\partial\Sigma$ using automatic differentiation.
        The target is total variance (not AAL), because $\partial\text{AAL}/\partial\Sigma = 0$ by
        design — the mean loss depends only on C (mean MDR), not on $\Sigma$.
        ...
        grad_Sigma : tf.Tensor, shape (K, M), dtype float32
            ...
            $\text{grad\_Sigma}[k,m] = \partial(\text{Var}_{\text{total\_portfolio}})/\partial\Sigma[k,m]$
        metrics : dict
        ...
        Notes
        -----
        - $\partial\text{AAL}/\partial\Sigma = 0$ is mathematically correct (mean is independent of variance)
        - This gradient targets variance: identifies which $\Sigma$ entries most
          affect total portfolio risk dispersion
        - Positive gradient means increasing $\Sigma[k,m]$ increases total variance
    ...
    def full_gradient_analysis(self) -> Dict:
        ...
        Calculates the full gradient vector $\nabla J = [\partial J/\partial H, \partial J/\partial C, \partial J/\partial v, \partial J/\partial\lambda]$ in a single
        ...
        analysis : dict
            ...
            grad_hazard : tf.Tensor, shape (N, Q), dtype float32
                $\partial(\text{AAL})/\partial H$ - Hazard intensity sensitivity (rate-weighted)
                
            grad_vulnerability : tf.Tensor, shape (K, M), dtype float32
                $\partial(\text{AAL})/\partial C$ - Vulnerability curve sensitivity (rate-weighted)
                
            grad_exposure : tf.Tensor, shape (N,), dtype float32
                $\partial(\text{AAL})/\partial v$ - Exposure sensitivity (rate-weighted)
                
            grad_lambdas : tf.Tensor, shape (Q,), dtype float32
                $\partial(\text{AAL})/\partial\lambda$ - Scenario occurrence rate sensitivity

            grad_sigma : tf.Tensor, shape (K, M), dtype float32, optional
                $\partial(\text{Var}_{\text{total}})/\partial\Sigma$ - Vulnerability uncertainty sensitivity.
            ...
            loss_matrix : tf.Tensor, shape (N, Q), dtype float32
                Complete loss matrix $J \in \mathbb{R}^{N \times Q}$
...
def classical_loss(v: tf.Tensor, u: tf.Tensor, C: tf.Tensor,
    ...
    $$\text{AAL}_i = v_i \times \int \text{MDR}(\text{iml}) \times |d\text{PoE}/d\text{iml}|\, d\text{iml}$$

    $$\approx v_i \times \sum_l \text{MDR}(\text{iml}_l) \times (\text{PoE}_{l-1} - \text{PoE}_l)$$

    where PoE values decrease with increasing IML (exceedance probabilities),
    so $(\text{PoE}_{l-1} - \text{PoE}_l)$ represents the probability mass in each IML bin.
    ...
    u : tf.Tensor, shape (N,), dtype int32
        Typology index per asset, values in {0, ..., K-1}.
    C : tf.Tensor, shape (K, M), dtype float32
        Vulnerability matrix (K curves $\times$ M curve points).
    x_grid : tf.Tensor, shape (M,), dtype float32
...
def consequence_loss(damage_probs: tf.Tensor, consequence_ratios: tf.Tensor,
    ...
    $$\text{loss}[i,q] = v[i] \times \sum_d \text{damage\_probs}[i,q,d] \times \text{consequence\_ratios}[u[i],d]$$
...
def classical_damage(u: tf.Tensor, F: tf.Tensor, x_grid: tf.Tensor,
    ...
    $$E[P(ds_d)] = \int P(ds_d | \text{iml}) \times |d\text{PoE}/d\text{iml}|\, d\text{iml}$$

    using the same $\Delta\text{PoE}$ discretization as classical_loss.
...
def benefit_cost_ratio(aal_original: tf.Tensor, aal_retrofitted: tf.Tensor,
    ...
    $$\text{BCR}_i = (\text{AAL}_{\text{original},i} - \text{AAL}_{\text{retrofitted},i}) \times \text{BPVF} / \text{retrofit\_cost}_i$$

    where $\text{BPVF} = (1 - (1+r)^{-T}) / r$ is the Benefit Present Value Factor.
