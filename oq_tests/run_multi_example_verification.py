...
LIB_DIR = ROOT / 'T-Hazard' / 'end_to_end_verification' / 'V2_event_based_risk'
...
def compute_trisk_from_oq(example_name: str, example_dir: Path, calc_id: int, vuln_files: dict):
    ...
    tr_total = np.zeros_like(h0, dtype=np.float64)
    used_components = []
    h_cache = {}
    ...
    for comp, vf in vuln_files.items():
        ...
        tr_total += component_loss
        used_components.append(comp)

    if not used_components:
        raise RuntimeError(f'{example_name}: no matching loss components found for translation')

    lambdas = compute_uniform_event_rates(events)
    tr_asset = (tr_total * lambdas[np.newaxis, :]).sum(axis=1)
    tr_event = tr_total.sum(axis=0)

    tr_asset_df = pd.DataFrame({'asset_id': asset_site['id'].astype(str).values, 'loss_tr_asset': tr_asset})
    tr_event_df = pd.DataFrame({'event_id': event_ids, 'loss_tr': tr_event})

    oq_event_df = select_oq_event_loss(rbe, used_components)
    oq_asset_df = select_oq_asset_loss(avg_losses, used_components)

    oq_event_aligned = tr_event_df[['event_id']].merge(oq_event_df, on='event_id', how='left').fillna(0.0)
    ...
    evt_cmp = tr_event_df.merge(oq_event_df, on='event_id', how='left').fillna(0.0)
    ast_cmp = tr_asset_df.merge(oq_asset_df, on='asset_id', how='left').fillna(0.0)

    evt_cmp['abs_diff'] = (evt_cmp['loss_tr'] - evt_cmp['loss_oq']).abs()
    ast_cmp['abs_diff'] = (ast_cmp['loss_tr_asset'] - ast_cmp['loss_oq_asset']).abs()
    ast_cmp['ratio'] = np.where(ast_cmp['loss_oq_asset'] > 0, ast_cmp['loss_tr_asset'] / ast_cmp['loss_oq_asset'], np.nan)

    mae = float(np.mean(np.abs(evt_cmp['loss_tr'] - evt_cmp['loss_oq'])))
    rmse = float(np.sqrt(np.mean((evt_cmp['loss_tr'] - evt_cmp['loss_oq']) ** 2)))
    corr = float(np.corrcoef(evt_cmp['loss_tr'].values, evt_cmp['loss_oq'].values)[0, 1]) if len(evt_cmp) > 1 else np.nan
    ss_res = float(np.sum((evt_cmp['loss_tr'] - evt_cmp['loss_oq']) ** 2))
    ss_tot = float(np.sum((evt_cmp['loss_oq'] - evt_cmp['loss_oq'].mean()) ** 2))
    r2 = 1.0 - ss_res / ss_tot if ss_tot > 0 else np.nan

    tr_port = float(np.sum(tr_event * lambdas))
    ...
    summary = {
        ...
        'portfolio_tr': tr_port,
        'portfolio_oq': oq_port,
        'portfolio_ratio': (tr_port / oq_port) if oq_port > 0 else np.nan,
        'event_mae': mae,
        'event_rmse': rmse,
        'event_corr': corr,
        'event_r2': r2,
...
def make_plots(example_name: str, ast_cmp: pd.DataFrame, evt_cmp: pd.DataFrame):
    ...
    w = 0.38
    ax.bar(x - w / 2, plot_assets['loss_oq_asset'], w, label='OQ', color='steelblue')
    ax.bar(x + w / 2, plot_assets['loss_tr_asset'], w, label='T-Risk translated', color='tomato')
    ax.set_xticks(x)
    ...
    eps = 1.0
    ax.scatter(evt_cmp['loss_oq'] + eps, evt_cmp['loss_tr'] + eps, s=8, alpha=0.35, color='darkgreen')
    lo = float(min((evt_cmp['loss_oq'] + eps).min(), (evt_cmp['loss_tr'] + eps).min()))
    hi = float(max((evt_cmp['loss_oq'] + eps).max(), (evt_cmp['loss_tr'] + eps).max()))
    ax.plot([lo, hi], [lo, hi], 'k--', lw=1.4)
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel('OQ event loss + 1')
    ax.set_ylabel('T-Risk event loss + 1')
...
def write_report_tex(summaries: pd.DataFrame):
    ...
    lines.append('\\title{Multi-Example OQ-to-T-Risk Verification}')
    lines.append('\\author{Automated verification}')
    lines.append('\\date{2026-02-19}')
    lines.append('\\begin{document}')
    lines.append('\\maketitle')
    lines.append('\\tableofcontents')
    lines.append('\\section{Objective}')
    lines.append('Run several OQ-engine risk examples end-to-end, translate inputs/outputs via \\texttt{library\\_oq\\_import.py}, and compare translated T-Risk calculations against OQ reference outputs.')
    ...
    for _, r in summaries.iterrows():
        ...
        lines.append(f"\\item Portfolio ratio (T-Risk/OQ): {r['portfolio_ratio']:.6f}")
    ...
    (BASE_OUT / 'oq_multi_example_verification_report.tex').write_text('\n'.join(lines), encoding='utf-8')
...
def main():
    ...
    for name, job in EXAMPLES:
        example_dir = OQ_ENGINE / 'demos' / 'risk' / name
        print(f'\\n=== Running {name} ({job}) ===')
        ...
        summary, ast_cmp, evt_cmp = compute_trisk_from_oq(name, example_dir, calc_id, vuln_files)
